...
export abstract class Basepage {
    ...
    protected getLocator(
        locatorDef: Record<string, string>,
        customeText: string
    ): Locator {
        const strategies = [
            {
            ...
            },
            {
                ...
                execute: () =>
                    this.page.getByRole(locatorDef.role as any, {
                        name:
                            locatorDef.text ||
                            customeText ||
                            locatorDef.name ||
            ...
            },
            {
                ...
                execute: () =>
                    this.page.getByPlaceholder(locatorDef.placeholder!),
            },
            {
                name: 'getByAltText',
            ...
            },
            {
                ...
                execute: () =>
                    this.page.getByText(locatorDef.text!, { exact: true }),
            },
            {
                name: 'getByCss',
        ...
        for (const strategy of strategies) {
            if (strategy.condition()) {
                ...
                try {
                    if (locator) {
                        console.log(
                            `Locator found using strategy: ${strategy.name}`
                        );
                        return locator;
                    }
                } catch (error) {
                    console.log(
                        `Error finding locator using strategy: ${strategy.name}`,
                        error
                    );
                    continue;
                }
        ...
        const availableStrategies = strategies
            .filter(
                (s) =>
                    locatorDef[s.name] ||
                    Object.keys(locatorDef).some((k) =>
                        k.includes(s.name.toLocaleLowerCase())
                    )
            )
            .map((s) => s.name);
